...
> Detailed templates for optional `.ai/rules/` files. Only create these if the project has established conventions for the domain. Do not generate empty or speculative rules.
...
- Reference task ID: include `Task <ID> from tasks/BACKLOG.md` in body
...
{{Include the PR body template from tasks/README.md or customize it here}}
...
4. **Link new rule files** in the appropriate agent pointer files (`AGENTS.md` and any tool-specific pointers selected by the user)
